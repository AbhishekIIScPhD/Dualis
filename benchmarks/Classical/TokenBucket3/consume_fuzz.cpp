...
int main(int argc, char* argv[]) {
	...
	if (fuzzLen < 8) {
        ...
        READ_UINT8_FROM_FUZZBUF(fuzzBuf, fuzzLen - 2, c_rate);
        READ_UINT8_FROM_FUZZBUF(fuzzBuf, fuzzLen - 1, b_size);

        TB_CONSUME_WITH_STATE(bucket, c_rate, b_size);
        
        bool expr = (((avai_tokens - avai_tokens1) - c_rate) <= 0); // Logic preserved
